...
const Results = ({ pageTitle }: { pageTitle: string }) => {
  ...
  const testsSkillTransformed = skillTests.map((test) => ({
    ...test,
    test_type: skillsIndex[test.test_type - 1],
  }));

  return (
    <div className="flex grid w-full max-w-6xl grid-cols-1 flex-col gap-[19px] md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-dashboard-layout">
      <DataTable
